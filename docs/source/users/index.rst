...
#  **Python eggs** are the Python equivalent to distribution packages such as
   Java Jars or Zip archives. Eggs can be installed and also declare
   dependencies to other packages and other metadata.

#  The **Zope Component Architecture** is a term for modularized software. It came
   into being when
...
#  Dexterity is the new kid on the block (not so new), ready to replace the
...
#  Buildout is the Python equivalent of Make or Ant: repeatable installations.
...
#  While Zope (and Plone) use the ZODB, an object database, some applications
   ...
   of **SQL** databases through an object relational mapper (**ORM**) like SQLAlchemy.
